...
# +
deploy_name = 'dfo-bb046-20201006'
deploy_prefix = '/Users/jklymak/gliderdata/deployments/dfo-bb046/dfo-bb046-20201006/'
# !mkdir figs
...
with get_gridfile(level='L1') as ds:
    ...
    fig, ax = plt.subplots()
    ds.salinity.plot(vmin=31, vmax=34.5)
    ax.set_ylim(1000, 0)
...
time = (ts.time.values-np.datetime64('2020-01-01')).astype('float') / 1e9
print(time[1:10])
ax.plot(ts.time[1:], np.diff(ts.pressure)/np.diff(time))
...
ax = axs[0]
pc = ax.pcolormesh(np.abs(ds.pitch), cmap='RdBu_r', vmin=10, vmax=30)
